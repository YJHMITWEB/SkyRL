...
def test_batched_moe_tensor_uses_one_full_expert_loader_call():
    ...
    wire_name = (
        f"{SKYRL_BATCHED_MOE_FP8_PREFIX}model.layers.0.mlp.experts.gate_proj.weight"
    )
...
def test_batched_moe_scale_maps_to_fused_scale_parameter():
    ...
    mapper = SimpleNamespace(
        apply_list=lambda names: [
            names[0].replace("model.language_model.", "language_model.model.", 1)
        ]
    )
    model = SimpleNamespace(hf_to_vllm_mapper=mapper)
    wire_name = (
        f"{SKYRL_BATCHED_MOE_FP8_PREFIX}"
        "model.language_model.layers.2.mlp.experts.up_proj.weight_scale_inv"
    )

    assert _load_batched_moe_fp8_tensor(
        model, {target_name: param}, wire_name, loaded_weight
    )
